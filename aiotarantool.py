...
def connect(host, port, user=None, password=None, loop=None,
            encoding=ENCODING_DEFAULT):
    conn = Connection(host, port, user=user, password=password,
                      loop=loop, encoding=encoding)
...
class Schema(object):
    ...
    def get_space(self, space):
        ...
        with (yield from self.con.lock):
            ...
            array = yield from self.con.select(
                tarantool.const.SPACE_SPACE, space, index=_index)
            if len(array) > 1:
                raise SchemaError(
    ...
    def get_index(self, space, index):
        ...
        with (yield from self.con.lock):
            ...
            array = yield from self.con.select(
                tarantool.const.SPACE_INDEX, [_space.sid, index], index=_index)
...
class Connection(tarantool.Connection):
            ...
            while len_buf - curr >= 5:
                ...
                try:
                    ...
                    if self.encoding is not None:
                        unpacker = msgpack.Unpacker(
                            use_list=True, encoding=self.encoding)
                    else:
                ...
                if not waiter.cancelled():
                    if response.return_code != 0:
                        waiter.set_exception(DatabaseError(
    ...
    def __repr__(self):
        return "aiotarantool.Connection(host=%r, port=%r)" % (
            self.host, self.port)
    ...
    def call(self, func_name, *args):
        ...
        resp = yield from self._send_request(
            RequestCall(self, func_name, args))
        return resp
    ...
    def replace(self, space_name, values):
        ...
        resp = yield from self._send_request(
            RequestReplace(self, space_name, values))
        return resp
    ...
    def _authenticate(self, user, password):
        ...
        resp = yield from self._send_request_no_check_connected(
            RequestAuthenticate(self, self._salt, user, password))
        return resp
    ...
    def insert(self, space_name, values):
        ...
        res = yield from self._send_request(
            RequestInsert(self, space_name, values))
        return res
    ...
    def select(self, space_name, key=None, **kwargs):
        ...
        res = yield from self._send_request(RequestSelect(
            self, space_name, index_name, key,
            offset, limit, iterator_type))
